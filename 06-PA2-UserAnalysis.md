...
| **N30** | Problem  | P09 (see also N20/N21/N24/N28/N29)           | Schedule only shows date and location; dedicated followers cannot find match time, participants, format, or bracket info, and searched elsewhere (including an external site) before giving up.|
| **N31** | Idea     | Brainstorm; P09                              | Surface richer match context (time, players, format, bracket) directly within Schedule items via an expandable popup or dedicated calendar view.|
...
* **Core Problem:** 61.1% of users reported difficulty finding specific info when they don't remember exact names. Long rating lists require 20+ seconds of vertical scrolling without a sticky search bar or pagination. This same theme extends beyond the Rating/News lists to the Schedule section: a dedicated follower (P09) actively searched the site for match time, participants, and bracket context that simply isn't surfaced anywhere, and gave up only after also checking an external source (N30).
* **Generated Ideas:**
  ...
  * Surface richer match context (time, players, format, bracket) directly within Schedule items so dedicated followers don't need to search elsewhere (N31).
...
5. **P-05 (Schedule lacks sufficient match/tournament information - Score 10/20)**:
   ...
   * *Severity (4/5)*: P09 actively searched the Schedule page, the tournament's own info page, and even followed an external link before giving up entirely — indicating a real, unmet need rather than a passing inconvenience (N30, N20, N21, N24, N28, N29).
...
The user analysis phase successfully consolidated the raw empirical data from Requirement 1 into four clear affinity clusters, prioritized the key problems using a structured score matrix, and formulated four final Problem Statements. 
